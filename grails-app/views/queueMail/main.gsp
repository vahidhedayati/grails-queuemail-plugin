<%@ page import="org.grails.plugin.queuemail.validation.ChangeConfigBean;org.grails.plugin.queuemail.EmailQueue;org.grails.plugin.queuemail.validation.QueueMailBean" %>
<!doctype html>
<html>
	<head>
...
<script>
	function reloadPage() {
		var url="${createLink(controller: 'queueMail',action:'listQueue')}";
		$.ajax({
			cache:false,			
			timeout:1000,
			type: 'POST',
	...
	function doDelete(id) {
		if (confirm('${message(code: 'queuemail.deleteWarning.message')}')) {
		 	ajaxCall('D',id)
		}
	}
	...
	function doDisplay(id,queueType) {
		...
		var title = "${g.message(code: 'queuemail.showRecord.message')}";
		return showDialog('${createLink(controller:'queueMail',action:'display')}?'+params,title);
	}
	function doPriority(id) {
		var params=$.param({'queue.id':id});
		var title = "${g.message(code: 'queuemail.changePriority.label')}";
		return showDialog('${createLink(controller:'queueMail',action:'changePriority')}?'+params,title);
	}
	function doConfigChange(id,type) {
		var params=$.param({'queue.id':id, changeType:type});
		var title = "${g.message(code: 'queuemail.changeConfig.label')}";
		return showDialog('${createLink(controller:'queueMail',action:'changeConfig')}?'+params,title);
	}
	...
	function postAction(method) {
		var url="${createLink(controller: 'queueMail',action:'deleteAll')}/?deleteBy="+method;			
		$.ajax({
			type: 'POST',
			url: url,
			data: $('#search').serialize(),
